...
public class CoordinacionServiceImpl implements CoordinacionService {
    ...
    private static final String CONVOCATORIA_ACTIVA = "1";
    ...
    public void savePreload(RelacionConvocatoriaCoordinacionDTO dto) {
        ...
        System.out.println("[DEBUG] Carga: " + carga);
    ...
    private void validateSavePreload(RelacionConvocatoriaCoordinacionDTO dto) {
        if (dto.idCoordinacion() == null || dto.idConvocatoria() == null) {
            throw new ApiException(
                    ...
                    "La coordinacion y la convocatoria son obligatorias");
        }
        ...
        ConvocatoriaEntity convocatoria = convocatoriaRepository
                .findConvocatoriaByIdNative(dto.idConvocatoria())
                .orElseThrow(() -> new ApiException(
                        HttpStatus.NOT_FOUND,
                        "Convocatoria no encontrada"));

        if (!CONVOCATORIA_ACTIVA.equals(convocatoria.getEstado())) {
            throw new ApiException(
                    HttpStatus.BAD_REQUEST,
                    "La convocatoria no esta activa");
        }

        if (cargaRepository.existsByIdCoordinacion(dto.idCoordinacion())) {
            throw new ApiException(
                    HttpStatus.CONFLICT,
                    "La coordinacion ya tiene una carga asignada");
        }

        if (cargaRepository.existsByIdCoordinacionAndIdConvocatoria(
                dto.idCoordinacion(),
                dto.idConvocatoria())) {
            throw new ApiException(
                    HttpStatus.CONFLICT,
                    "La coordinacion ya tiene carga en esta convocatoria");
        }
    }
}
